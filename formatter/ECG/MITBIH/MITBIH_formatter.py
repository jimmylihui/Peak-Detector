...
class FetalECGDatasetFormatter:
    ...
    def __init__(self, segment_length: int = 1000, sampling_rate: int = 360, use_special_tokens: bool = True):
        ...
        self.sampling_rate = sampling_rate    # MIT-BIH sampling rate is 360 Hz
    ...
    def detect_input_peaks(self, signal: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            min_distance = 20  # 300ms minimum between peaks
    ...
    def calculate_error_metrics(self, original: np.ndarray, reconstructed: np.ndarray, peaks: np.ndarray = None) -> Dict[str, float]:
        ...
        --------
        Dict[str, float]
            Dictionary with mae, rmse, and mape values
        """
    ...
    def extract_segments(self, ecg_data: pd.DataFrame, annotations: pd.DataFrame, lead: str = 'MLII') -> List[Dict]:
        ...
        for start_idx in range(0, total_samples - self.segment_length + 1, self.segment_length):
            ...
            segment_data = {
                ...
                'ecg_signal': normalized_segment.tolist(),
    ...
    def create_instruction_entry(self, segment_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        signal = np.array(segment_data['ecg_signal'])
        ...
        instruction = ("You are a specialized assistant for biomedical signal analysis, specifically trained in ECG R-peak detection. "
                      "Analyze the following ECG signal peaks using R-peak detection methodology. "
                      "Peak detection guidance: R-peaks are the prominent positive deflections in ECG signals corresponding to ventricular depolarization. "
                      "Pay attention to the timestamp, the time interval is stable for each peaks. "
                      "Output format: Structured format with R-peak positions in brackets.")
        
        input_text = f"mitbih ECG signal (MLII lead) sampled at {self.sampling_rate} Hz with duration of {duration_seconds:.2f} seconds. Detected peaks in signal: {input_peaks_text}"
    ...
    def load_and_format_mitbih_dataset(self, data_dir: str, record_numbers: List[str], 
                                      output_dir: str = "formatted_mitbih_dataset", 
                                      lead: str = 'MLII') -> None:
        """Load and format MIT-BIH dataset."""
        print(f"=== MIT-BIH DATASET FORMATTING ===")
        print(f"Segment length: {self.segment_length} samples")
        print(f"Sampling rate: {self.sampling_rate} Hz")
        print(f"ECG Lead: {lead}")
        ...
        for record_num in record_numbers:
            try:
                # Load MIT-BIH record
                ecg_data, annotations = self.load_mitbih_record(record_num, data_path)
                
                # Check if record has any R peaks at all
                if len(annotations) == 0:
                    print(f"Skipping record {record_num}: No R peaks found in annotations")
                    continue
                
                # Process record data
                processed_segments = self.process_record_data(record_num, ecg_data, annotations, lead)
                
                # Skip record if no segments were generated (all segments had 0 R peaks)
                if len(processed_segments) == 0:
                    print(f"Skipping record {record_num}: No segments with R peaks found")
                    continue
                ...
                for segment_data in processed_segments:
                    ...
                    record_examples.append(instruction_entry)
                
                all_examples.extend(record_examples)
                
                # Save individual record file
                record_filename = os.path.join(output_dir, f"record_{record_num}.json")
                with open(record_filename, 'w') as f:
                    json.dump(record_examples, f, indent=2)
                
                print(f"Saved {len(record_examples)} examples to {record_filename}")
        ...
        combined_filename = os.path.join(output_dir, "mitbih_ecg_peaks.json")
...
def main():
    ...
    # Load processed data
    training_segments = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_X_train.npy')
    test_segments = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_X_test.npy')
   
    
    training_labels = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_y_train.npy')
    test_labels = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_y_test.npy')
    ...
    formatter = FetalECGDatasetFormatter(segment_length=1000, sampling_rate=360)
    ...
    output_dir = "/path/to/workspace/project-BCG-LLM/combined_data/format_data/mitbih_ecg_peaks"
    os.makedirs(output_dir, exist_ok=True)
    
    # Save training data
    training_filename = os.path.join(output_dir, "mitbih_ecg_peaks_train.json")
    with open(training_filename, 'w') as f:
        json.dump(training_examples, f, indent=2)
    print(f"Saved {len(training_examples)} training examples to {training_filename}")
    
    # Save test data
    test_filename = os.path.join(output_dir, "mitbih_ecg_peaks_test.json")
    ...
    combined_filename = os.path.join(output_dir, "mitbih_ecg_peaks_combined.json")
    with open(combined_filename, 'w') as f:
        json.dump(all_examples, f, indent=2)
