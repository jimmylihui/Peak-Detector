...
class FetalECGDatasetFormatter:
    """
    Formats fetal ECG signal data for instruction finetuning.
    ...
    def detect_input_peaks(self, signal: np.ndarray) -> np.ndarray:
        ...
        try:
            pos_peaks, _ = find_peaks(signal, distance=20)
            neg_peaks, _ = find_peaks(-signal, distance=20)
    ...
    def create_instruction_entry(self, segment_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        instruction = ("You are a specialized assistant for biomedical signal analysis, specifically trained in fetal ECG R-peak detection. "
        ...
        input_text = f"Incart ECG signal ({segment_data['lead']} lead) sampled at {self.sampling_rate} Hz with duration of {duration_seconds:.2f} seconds. Detected peaks in signal: {self.format_peaks_for_input(input_peaks, signal)}"
    ...
    def load_and_format_mitbih_dataset(self, data_dir: str, record_numbers: List[str], 
        ...
        combined_filename = os.path.join(output_dir, "incart_ecg_peaks.json")
...
def main():
    ...
    base_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG_incart'
    
    # Load processed data
    training_segments = np.load(f'{base_path}/total_X_train.npy')
    test_segments = np.load(f'{base_path}/total_X_test.npy')
    training_labels = np.load(f'{base_path}/total_y_train.npy')
    test_labels = np.load(f'{base_path}/total_y_test.npy')
    ...
    formatter = FetalECGDatasetFormatter(segment_length=1000, sampling_rate=257)
    ...
    output_dir = "/path/to/workspace/project-BCG-LLM/combined_data/format_data/incart_ecg_peaks"
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Save all datasets
    datasets = [
        ("incart_ecg_peaks_train.json", training_examples),
        ("incart_ecg_peaks_test.json", test_examples),
        ("incart_ecg_peaks_combined.json", all_examples)
    ]
