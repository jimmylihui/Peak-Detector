...
def neurokit2_detect(ecg: np.ndarray, fs: int = 360, method: str = 'pantompkins1985') -> np.ndarray:
    ...
    try:
        ...
        # Detect R-peaks
        _, rpeaks = nk.ecg_peaks(ecg, sampling_rate=fs, method=method)
...
def process_sample(args):
    ...
    if mode == 'ensemble':
        ...
        pred_peaks = neurokit2_detect(preprocess_signal(sig), fs=fs, method='nabian2018')
...
def main():
    ...
    # 'single' - uses nabian2018 algorithm
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'arrhy_predicted_peaks.npy')
