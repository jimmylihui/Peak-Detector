...
def calculate_ibi_from_peaks(peaks: np.ndarray, fs: int = 360) -> float:
    """Calculate mean Inter-Beat Interval (IBI) from peaks in milliseconds."""
    if peaks is None or len(peaks) < 2:
        return np.nan
    rr_intervals = np.diff(peaks) / fs * 1000.0  # Convert to milliseconds
    if len(rr_intervals) == 0:
        return np.nan
...
def main():
    ...
    for i in tqdm(range(X_test.shape[0]), desc='Testing with NeuroKit2'):
        ...
        if detection_mode == 'ensemble':
            ...
            pred_peaks = neurokit2_detect(signal_np, fs=fs, method='nabian2018')
        ...
        if len(pred_peaks) > 0:
            ...
            pred_ibi = calculate_ibi_from_peaks(pred_peaks, fs=fs)
            gt_ibi = calculate_ibi_from_peaks(gt_peaks, fs=fs)
            if not (np.isnan(pred_ibi) or np.isnan(gt_ibi)):
                ibi_errors.append(abs(pred_ibi - gt_ibi))
    ...
    ibi_mae = float(np.mean(ibi_errors)) if len(ibi_errors) > 0 else float('nan')
    ...
    print(f'IBI MAE (ms): {ibi_mae:.2f}' if not np.isnan(ibi_mae) else 'IBI MAE (ms): NaN')
