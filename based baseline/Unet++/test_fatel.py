...
def main():
	...
	model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'unet_plus_plus_fatel_trained_model.pt')
	...
	def reshape_segments(X, y, segment_length=2000):
		"""
		Reshape data from (n, 10000, ...) to (5*n, 2000, ...)
		X: (n, 10000, ...) (e.g., (n, 10000, 2))
		y: (n, 10000, ...) (e.g., (n, 10000))
		Returns:
			X_new: (5*n, 2000, ...)
			y_new: (5*n, 2000, ...)
		"""
		...
		# Reshape X
		X_new = X.reshape(n, num_segments, segment_length, *X.shape[2:]).reshape(n * num_segments, segment_length, *X.shape[2:])
	...
	X_test, y_test = reshape_segments(X_test, y_test)
	...
	model = UNetPlusPlus1D(input_channels=1, base_filters=16).to(device)
	...
	for i in tqdm(range(X_test.shape[0]), desc='Testing'):
		signal_np = preprocess_signal(X_test[i,:,1])
		label_np = y_test[i]
