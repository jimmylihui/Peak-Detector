...
class DeepLearningMitbihBenchmark:
    ...
    def load_dataset(self):
        ...
        X_BCG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/X_train.npy')
        ...
        y_BCG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/y_train.npy')
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        X_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/X_test.npy')
        y_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/y_test.npy')
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_BSG_Arrhythmia_predicted_peaks.npy')
    ...
    def train_and_evaluate_test_folds(self, epochs=100, batch_size=32, learning_rate=0.001):
        ...
        # Save predicted peaks
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_BSG_Arrhythmia_test_folds_predicted_peaks.npy')
        predicted_peaks_array = np.array(all_predicted_peaks_binary, dtype=np.float32)
        np.save(output_path, predicted_peaks_array)
        print(f'\nPredicted peaks saved to: {output_path}')
        print(f'Shape: {predicted_peaks_array.shape}')
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='unet_plus_plus_bsg_arrhythmia_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    print("UNet++ ECG R-peak Detection - Cross-Validation Pipeline (BSG Arrhythmia)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
...
def main_train_and_evaluate_test_folds():
    ...
    print("UNet++ ECG R-peak Detection - Train on Training Data, Evaluate on Test Folds (BSG Arrhythmia)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
    print(f"Epochs: {epochs}, Batch size: {batch_size}, Learning rate: {learning_rate}")
