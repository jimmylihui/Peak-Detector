...
def calculate_metrics(pred_peaks, gt_peaks, tolerance=0.03, fs=1000):
    """
...
class DeepLearningMitbihBenchmark:
    ...
    def load_dataset(self):
        X_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG_fatel/segments_train.npy')[:,:,1]

        y_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG_fatel/maternal_gaussian_train.npy')
        from sklearn.model_selection import train_test_split
        X_ECG_train, X_ECG_val, y_ECG_train, y_ECG_val = train_test_split(
            X_ECG_train, y_ECG_train, test_size=0.2, random_state=42
        )
        # Reshape data from (n,10000) to (5*n,2000)
        def reshape_segments(X, y, segment_length=2000):
            """
            Reshape data from (n, 10000) to (5*n, 2000).
            X: (n, 10000)
            y: (n, 10000) or (n, 10000, ...)
            Returns:
                X_new: (5*n, 2000)
                y_new: (5*n, 2000) or (5*n, 2000, ...)
            """
            n = X.shape[0]
            total_length = X.shape[1]
            num_segments = total_length // segment_length
            # Reshape X
            X_new = X.reshape(n, num_segments, segment_length).reshape(n * num_segments, segment_length)
            # Reshape y (handle extra dims)
            y_shape = y.shape
            if len(y_shape) == 2:
                y_new = y.reshape(n, num_segments, segment_length).reshape(n * num_segments, segment_length)
            else:
                # e.g., (n, 10000, 1)
                trailing_shape = y_shape[2:]
                y_new = y.reshape(n, num_segments, segment_length, *trailing_shape).reshape(n * num_segments, segment_length, *trailing_shape)
            return X_new, y_new

        X_ECG_train, y_ECG_train = reshape_segments(X_ECG_train, y_ECG_train)
        X_ECG_val, y_ECG_val = reshape_segments(X_ECG_val, y_ECG_val)
...
def main():
    ...
    # Configuration
    window_length = 10000  # Non-overlapping windows of 1000 samples (UNet++ requirement)
    tolerance = int(0.03*1000)  # 3 positions tolerance in samples
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='unet_plus_plus_fatel_trained_model.pt',
        epochs=epochs,
