...
def calculate_hrv_rmssd_from_peaks(peaks: np.ndarray, fs: int = 360) -> float:
    """Calculate HRV RMSSD from peaks in milliseconds."""
...
class DeepLearningMitbihBenchmark:
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        X_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_X_test.npy')
        y_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_y_test.npy')
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_predicted_peaks.npy')
    ...
    def load_dataset(self):
        X_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_X_train.npy')

        y_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/total_y_train.npy')
        from sklearn.model_selection import train_test_split, KFold
        X_ECG_train, X_ECG_val, y_ECG_train, y_ECG_val = train_test_split(
            X_ECG_train, y_ECG_train, test_size=0.2, random_state=42
        )
    ...
    def calculate_ibi_from_peaks(self, peaks, fs=360):
    ...
    def calculate_hr_from_peaks(self, peaks, fs=360):
    ...
    def calculate_hrv_from_peaks(self, peaks, fs=360, metric='sdnn'):
    ...
    def print_dataset_statistics(self):
        ...
        # Extract peaks from labels
        print("\nExtracting peaks from labels...")
        train_peaks = self.extract_peaks_from_labels(y_train)
        test_peaks = self.extract_peaks_from_labels(y_test)
        ...
        for peaks in train_peaks:
            if len(peaks) > 1:
                ibi = self.calculate_ibi_from_peaks(peaks, fs=self.fs)
                train_ibi_list.extend(ibi)
                hr = self.calculate_hr_from_peaks(peaks, fs=self.fs)
                if not np.isnan(hr):
                    train_hr_list.append(hr)
                hrv = self.calculate_hrv_from_peaks(peaks, fs=self.fs)
                if not np.isnan(hrv):
                    train_hrv_list.append(hrv)
            train_peak_counts.append(len(peaks))
        
        # Calculate statistics for test dataset
        print("Calculating test dataset statistics...")
        test_ibi_list = []
        test_hr_list = []
        test_hrv_list = []
        test_peak_counts = []
        ...
        for peaks in test_peaks:
            if len(peaks) > 1:
                ibi = self.calculate_ibi_from_peaks(peaks, fs=self.fs)
                test_ibi_list.extend(ibi)
                hr = self.calculate_hr_from_peaks(peaks, fs=self.fs)
                if not np.isnan(hr):
                    test_hr_list.append(hr)
                hrv = self.calculate_hrv_from_peaks(peaks, fs=self.fs)
                if not np.isnan(hrv):
                    test_hrv_list.append(hrv)
            test_peak_counts.append(len(peaks))
        ...
        print(f"Average IBI: {np.mean(train_ibi_list):.4f} seconds" if len(train_ibi_list) > 0 else "Average IBI: N/A")
        print(f"Mean HR: {np.mean(train_hr_list):.2f} BPM" if len(train_hr_list) > 0 else "Mean HR: N/A")
        print(f"Std HR: {np.std(train_hr_list):.2f} BPM" if len(train_hr_list) > 0 else "Std HR: N/A")
        print(f"Mean HRV: {np.mean(train_hrv_list):.2f} ms" if len(train_hrv_list) > 0 else "Mean HRV: N/A")
        print(f"Std HRV: {np.std(train_hrv_list):.2f} ms" if len(train_hrv_list) > 0 else "Std HRV: N/A")
        print(f"Average peak number: {np.mean(train_peak_counts):.2f}")
        
        print("\n" + "="*80)
        print("TEST DATASET STATISTICS")
        print("="*80)
        print(f"Number of samples: {len(y_test)}")
        print(f"Average IBI: {np.mean(test_ibi_list):.4f} seconds" if len(test_ibi_list) > 0 else "Average IBI: N/A")
        print(f"Mean HR: {np.mean(test_hr_list):.2f} BPM" if len(test_hr_list) > 0 else "Mean HR: N/A")
        print(f"Std HR: {np.std(test_hr_list):.2f} BPM" if len(test_hr_list) > 0 else "Std HR: N/A")
        print(f"Mean HRV: {np.mean(test_hrv_list):.2f} ms" if len(test_hrv_list) > 0 else "Mean HRV: N/A")
        print(f"Std HRV: {np.std(test_hrv_list):.2f} ms" if len(test_hrv_list) > 0 else "Std HRV: N/A")
        print(f"Average peak number: {np.mean(test_peak_counts):.2f}")
        print("="*80)
    def train_and_evaluate(self, model_save_path=None, epochs=100, batch_size=32, 
        ...
        test_metrics = self.test_throughput(X_val, y_val, batch_size=batch_size)
    ...
    def train_and_evaluate_test_folds(self, epochs=100, batch_size=32, learning_rate=0.001):
        ...
        # Save predicted peaks
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_mitbih_test_folds_predicted_peaks.npy')
        predicted_peaks_array = np.array(all_predicted_peaks_binary, dtype=np.float32)
        np.save(output_path, predicted_peaks_array)
        print(f'\nPredicted peaks saved to: {output_path}')
        print(f'Shape: {predicted_peaks_array.shape}')
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='unet_plus_plus_mitbih_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    print("UNet++ ECG R-peak Detection - Cross-Validation Pipeline")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
...
def main_train_and_evaluate_test_folds():
    ...
    # Configuration
    window_length = 1000  # Non-overlapping windows of 1000 samples (UNet++ requirement)
    tolerance = int(0.03*360)  # 3 positions tolerance in samples (MIT-BIH fs=360)
    ...
    print("UNet++ ECG R-peak Detection - Train on Training Data, Evaluate on Test Folds (MIT-BIH)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
    print(f"Epochs: {epochs}, Batch size: {batch_size}, Learning rate: {learning_rate}")
...
if __name__ == "__main__":
    ...
    # benchmark = DeepLearningMitbihBenchmark(window_length=1000, tolerance=3)
    # benchmark.print_dataset_statistics()  # Print dataset statistics
