...
class DeepLearningMitbihBenchmark:
    ...
    def __init__(self, model_path=None, window_length=1000, tolerance=3):
        ...
        self.fs = 100  # BCG_hospital sampling frequency
    ...
    def load_dataset(self):
        ...
        X_BCG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/X_train.npy')
         
        y_BCG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/y_train.npy')
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        X_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/X_test.npy')
        y_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/y_test.npy')
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_ICUBSG_predicted_peaks.npy')
    ...
    def train_and_evaluate_test_folds(self, epochs=100, batch_size=32, learning_rate=0.001):
        ...
        # Save predicted peaks
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'UNet++_ICUBSG_test_folds_predicted_peaks.npy')
        predicted_peaks_array = np.array(all_predicted_peaks_binary, dtype=np.float32)
        np.save(output_path, predicted_peaks_array)
        print(f'\nPredicted peaks saved to: {output_path}')
        print(f'Shape: {predicted_peaks_array.shape}')
...
def main():
    ...
    tolerance = int(0.03*100)  # 3 positions tolerance in samples
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='unet_plus_plus_ICUBSG_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    # Configuration
    window_length = 1000  # Non-overlapping windows of 1000 samples (UNet++ requirement)
    tolerance = int(0.03*100)  # 3 positions tolerance in samples (BCG_hospital fs=100)
    ...
    print("UNet++ ECG R-peak Detection - Cross-Validation Pipeline (ICU BSG)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
...
def main_train_and_evaluate_test_folds():
    ...
    print("UNet++ ECG R-peak Detection - Train on Training Data, Evaluate on Test Folds (ICU BSG)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
    print(f"Epochs: {epochs}, Batch size: {batch_size}, Learning rate: {learning_rate}")
