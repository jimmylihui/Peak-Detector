...
def calculate_mape(predicted: float, actual: float, min_threshold: float = 1.0) -> float:
    ...
    Args:
        ...
        min_threshold: Minimum threshold for actual value to avoid division by very small numbers.
                      For HRV (in ms), values below 5.0 ms are considered unreliable.
...
def process_sample(args):
    ...
    result = {'tp': 0, 'fp': 0, 'fn': 0, 'hr_error': None, 'hrv_sdnn_error': None, 'hrv_rmssd_error': None, 'hr_mape': None, 'hrv_sdnn_mape': None, 'hrv_rmssd_mape': None}
    if len(pred_peaks) > 0:
...
def main():
    # Config
    fs = 300
    window_length = 1000  # Capno dataset uses 2000 sample windows
