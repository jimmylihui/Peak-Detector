...
def calculate_detection_metrics(pred_peaks: np.ndarray, gt_peaks: np.ndarray, tolerance_samples: int, fs: int = 360, relative_tolerance: float = None) -> dict:
	"""
	Calculate detection metrics with optional relative tolerance based on GT heart rate.

	Args:
		pred_peaks: Predicted peak indices
		gt_peaks: Ground truth peak indices
		tolerance_samples: Fixed tolerance in samples (used if relative_tolerance is None)
		fs: Sampling frequency
		relative_tolerance: If provided, tolerance is calculated as a fraction of GT RR interval
	"""
	...
	# Calculate adaptive tolerance based on GT heart rate if relative_tolerance is provided
	if relative_tolerance is not None and len(gt_peaks) >= 2:
		gt_rr_intervals = np.diff(gt_peaks)
		avg_gt_rr = np.mean(gt_rr_intervals)
		tolerance_samples = int(avg_gt_rr * relative_tolerance)
		# Ensure minimum tolerance
		tolerance_samples = max(tolerance_samples, 1)
	elif relative_tolerance is not None and len(gt_peaks) < 2:
		# Fallback to fixed tolerance if not enough GT peaks for relative calculation
		tolerance_samples = max(int(0.03 * fs), 1)
...
def main():
	...
	# Option 1: Use fixed tolerance (set relative_tolerance to None)
	# tolerance_samples = int(0.03 * fs)
	# relative_tolerance = None

	# Option 2: Use relative tolerance based on GT heart rate (recommended)
	# relative_tolerance is a fraction of the GT RR interval
	relative_tolerance = 0.05  # 5% of GT RR interval
	tolerance_samples = None  # Will be calculated per sample
	
	model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sepconv_bidmc_trained_model.pt')
	...
	for i in tqdm(range(X_test.shape[0]), desc='Testing'):
		...
		# Detection metrics
		metrics = calculate_detection_metrics(pred_peaks, gt_peaks, tolerance_samples, fs=fs, relative_tolerance=relative_tolerance)
		all_tp += metrics['tp']
		all_fp += metrics['fp']
		all_fn += metrics['fn']
	...
	if relative_tolerance is not None:
		print(f'Tolerance: {relative_tolerance*100:.1f}% of GT RR interval (relative)')
	else:
		print(f'Tolerance: {tolerance_samples} samples (fixed)')
