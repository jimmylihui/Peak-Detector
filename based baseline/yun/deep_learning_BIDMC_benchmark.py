...
class DeepLearningMitbihBenchmark:
    ...
    def __init__(self, model_path=None, window_length=2048, tolerance=3):
        ...
        self.fs = 125  # BIDMC PPG sampling frequency
    ...
    def load_dataset(self):
        ...
        X_train_1 = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/processed/X_train.npy')
       
        y_train   = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/processed/Y_train.npy')
        
        from sklearn.model_selection import train_test_split
        X_train_1, X_val_1, y_train, y_val = train_test_split(
            X_train_1, y_train, test_size=0.2, random_state=42
        )
        ...
        X_train = make_two_channel(X_train_1)
        X_val   = make_two_channel(X_val_1)
    ...
    def calculate_ibi_from_peaks(self, peaks, fs=125):
    ...
    def calculate_hr_from_peaks(self, peaks, fs=125):
    ...
    def calculate_hrv_from_peaks(self, peaks, fs=125, metric='sdnn'):
    ...
    def localize_predicted_peaks(self, pred_probs, fs=125):
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        X_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/processed/X_test.npy')
        y_test = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/processed/Y_test.npy')
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SepConv_BIDMC_predicted_peaks.npy')
    ...
    def train_and_evaluate(self, model_save_path=None, epochs=100, batch_size=32, 
        ...
        if model_save_path is None:
            model_save_path = f'sepconv_bidmc_model_{timestamp}.pt'
...
def main():
    ...
    tolerance = int(0.03*125)  # 3 positions tolerance in samples
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='sepconv_bidmc_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    window_length = 2048
    tolerance = int(0.03*125)
    
    epochs = 50
    batch_size = 32
    learning_rate = 0.001
    
    print("SepConv ECG R-peak Detection - Cross-Validation Pipeline (BIDMC)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
