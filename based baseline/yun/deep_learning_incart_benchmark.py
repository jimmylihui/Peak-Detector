...
class DeepLearningMitbihBenchmark:
    ...
    def load_dataset(self):
        ...
        X_train = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG_incart/total_X_train.npy')

        y_train   = np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG_incart/total_y_train.npy')
        from sklearn.model_selection import train_test_split
    ...
    def localize_predicted_peaks(self, pred_probs, fs=257):
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SepConv_incart_predicted_peaks.npy')
    ...
    def train_and_evaluate(self, model_save_path=None, epochs=100, batch_size=32, 
        ...
        if model_save_path is None:
            model_save_path = f'sepconv_incart_model_{timestamp}.pt'
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='sepconv_incart_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    window_length = 2048
    tolerance = int(0.03*257)
    
    epochs = 5
    batch_size = 32
    learning_rate = 0.001
    
    print("SepConv ECG R-peak Detection - Cross-Validation Pipeline (Incart)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
