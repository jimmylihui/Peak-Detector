...
class DeepLearningMitbihBenchmark:
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SepConv_HospitalBSG_predicted_peaks.npy')
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='sepconv_hospitalbsg_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    print("SepConv ECG R-peak Detection - Cross-Validation Pipeline (Hospital BSG)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
