...
class DeepLearningMitbihBenchmark:
    ...
    def load_dataset(self):
        ...
        X_train = make_two_channel(X_BCG_train)
        X_val   = make_two_channel(X_BCG_val)
        ...
        y_train = make_labels(y_BCG_train)
        y_val   = make_labels(y_BCG_val)
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SepConv_ICUBSG_predicted_peaks.npy')
    ...
    def train_and_evaluate(self, model_save_path=None, epochs=100, batch_size=32, 
        ...
        if model_save_path is None:
            model_save_path = f'sepconv_hospitalbsg_model_{timestamp}.pt'
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='sepconv_ICUBSG_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    window_length = 2048
    tolerance = int(0.03*100)
    
    epochs = 5
    batch_size = 32
    learning_rate = 0.001
    
    print("SepConv ECG R-peak Detection - Cross-Validation Pipeline (ICU BSG)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
