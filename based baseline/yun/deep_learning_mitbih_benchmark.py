...
class DeepLearningMitbihBenchmark:
    ...
    def extract_peaks_from_labels(self, y_labels):
        ...
        for i in range(len(y_labels)):
            ...
            # For binary labels, find peaks with minimum height and distance
            refractory = int(self.fs * 0.2)
            peaks, _ = signal.find_peaks(signal_flat, height=0.5, distance=refractory)
            all_peaks.append(peaks)
        return all_peaks
    
    def localize_predicted_peaks(self, pred_probs, fs=360):
    ...
    def evaluate_fold(self, X_test, y_test, batch_size=32):
        ...
        Args:
            X_test: Test signals (already in 2-channel format)
            y_test: Test labels
        ...
        with torch.no_grad():
            ...
            for batch_idx, (batch_signals, batch_labels) in enumerate(test_loader):
                ...
                outputs = self.model(batch_signals)  # SepConv outputs logits
                ...
                for i in range(outputs_np.shape[0]):
                    ...
                    # Get predicted peaks from model output (SepConv outputs are (N, 1, L))
                    if outputs_np.ndim == 3:
                        pred_probs = outputs_np[i, 0, :].flatten()
                    else:
                        pred_probs = outputs_np[i].flatten()
                    
                    # Localize peaks using adaptive thresholding
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        # Process test data same way as train data
        def make_two_channel(arr: np.ndarray) -> np.ndarray:
            arr = np.ascontiguousarray(arr)
            if arr.ndim == 1:
                arr = arr[None, :]
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'SepConv_MITBIH_predicted_peaks.npy')
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='sepconv_mitbih_trained_model.pt',
        epochs=epochs,
...
def main_cross_validation():
    ...
    # Configuration
    window_length = 2048  # SepConv uses 2048-sample windows
    tolerance = int(0.03*360)  # 3 positions tolerance in samples
    ...
    print("SepConv ECG R-peak Detection - Cross-Validation Pipeline (MIT-BIH)")
    print("="*70)
    print(f"Window length: {window_length} samples")
    print(f"Tolerance: {tolerance} samples")
