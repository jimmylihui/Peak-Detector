...
class DeepLearningMitbihBenchmark:
    ...
    def __init__(self, model_path=None, window_length=1000, tolerance=3):
        ...
        if model_path is None:
            ...
            self.model_path = os.path.join(path_base, 'model', 'transformer_model.pt')
        else:
    ...
    def load_dataset(self):
        X_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/X_train.npy')
        
        
        y_ECG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/ECG/y_train.npy')
        ...
        X_PPG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/X_train.npy')
        X_PPG_val=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/X_val.npy')
        y_PPG_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/y_train.npy')
        y_PPG_val=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/y_val.npy')

        # Upsample each modality to match the largest split size
        target_train = max(X_ECG_train.shape[0], X_BCG_train.shape[0], X_PPG_train.shape[0])
        target_val = max(X_ECG_val.shape[0], X_BCG_val.shape[0], X_PPG_val.shape[0])

        X_ECG_train, y_ECG_train = self._upsample_to_size(X_ECG_train, y_ECG_train, target_train)
        X_BCG_train, y_BCG_train = self._upsample_to_size(X_BCG_train, y_BCG_train, target_train)
        X_PPG_train, y_PPG_train = self._upsample_to_size(X_PPG_train, y_PPG_train, target_train)

        X_ECG_val, y_ECG_val = self._upsample_to_size(X_ECG_val, y_ECG_val, target_val)
        X_BCG_val, y_BCG_val = self._upsample_to_size(X_BCG_val, y_BCG_val, target_val)
        X_PPG_val, y_PPG_val = self._upsample_to_size(X_PPG_val, y_PPG_val, target_val)

        X_train=np.concatenate((X_ECG_train, X_BCG_train, X_PPG_train), axis=0)
        X_val=np.concatenate((X_ECG_val, X_BCG_val, X_PPG_val), axis=0)
        y_train=np.concatenate((y_ECG_train, y_BCG_train, y_PPG_train), axis=0)
        y_val=np.concatenate((y_ECG_val, y_BCG_val, y_PPG_val), axis=0)
    ...
    def train_and_evaluate(self, model_save_path=None, epochs=100, batch_size=32, 
        ...
        if model_save_path is None:
            model_save_path = f'transformer_model_{timestamp}.pt'
...
def main():
    ...
    history = benchmark.train_and_evaluate(
        model_save_path='transformer_trained_model.pt',
        epochs=epochs,
