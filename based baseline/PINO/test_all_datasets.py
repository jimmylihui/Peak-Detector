#!/usr/bin/env python3
"""
PINO benchmark: Original vs Fixed on all datasets.

Compares:
  - PINO (Original): D3-D6 + D1 leak (from bcg-hr-dl)
  - PINO (Fixed):    D4-D7, no D1 leak (correct per paper)

Tolerance modes:
  - Absolute: 50 ms fixed window
  - Relative: 5% of median GT RR interval (min 5 samples)
"""
...
sys.path.insert(0, '/path/to/workspace/project-BCG-LLM/BSG_LLM/benchmark/bcg-hr-dl')
from algorithms import pino as pino_orig_module

from pino_fixed import pino_fixed
...
def detect_pino_orig(sig_data, fs):
    sig_data = np.ascontiguousarray(sig_data).astype(np.float64)
    try:
        peaks = pino_orig_module.pino(sig_data, f=fs)
    except Exception:
...
def detect_pino_fixed(sig_data, fs):
    sig_data = np.ascontiguousarray(sig_data).astype(np.float64)
    try:
        peaks = pino_fixed(sig_data, f=fs)
    except Exception:
        return np.array([], dtype=int)
    if peaks is None or len(peaks) == 0:
        return np.array([], dtype=int)
...
def _worker(args):
    sig_data, lbl, fs, abs_tol, det_name = args
    gt = extract_gt_peaks(lbl, fs)

    t0 = time.time()
    if det_name == 'pino_orig':
        pred = detect_pino_orig(sig_data, fs)
    else:
        pred = detect_pino_fixed(sig_data, fs)
...
def main():
    ...
    detectors = [
        ('pino_orig', 'PINO (Original)'),
        ('pino_fixed', 'PINO (Fixed)'),
    ]
    ...
    # Write combined result.md
    write_combined_result_md(all_results)


def write_combined_result_md(pino_results):
    """Merge PINO results with existing results into result.md."""
    ...
    elgendi_json = os.path.join(base_dir, '..', 'Elgendi', 'results.json')
    elgendi_results = {}
    if os.path.exists(elgendi_json):
        with open(elgendi_json) as f:
            elgendi_results = json.load(f)

    nk2_json = os.path.join(base_dir, '..', 'Elgendi', 'results_nk2.json')
    nk2_results = {}
    if os.path.exists(nk2_json):
        with open(nk2_json) as f:
            nk2_results = json.load(f)
    ...
    if elgendi_results:
        algos.append(('elgendi', 'Elgendi (Standalone)', elgendi_results))
    if nk2_results:
        algos.append(('elgendi_nk2', 'Elgendi (NeuroKit2)', nk2_results))
    algos.append(('pino_orig', 'PINO (Original)', pino_results))
    algos.append(('pino_fixed', 'PINO (Fixed)', pino_results))
