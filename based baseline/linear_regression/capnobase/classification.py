...
# Data paths
x_test_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG_capnobase/processed/X_test.npy'
y_test_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG_capnobase/processed/Y_test_gauss.npy'
x_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG_capnobase/processed/X_train.npy'
y_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG_capnobase/processed/Y_train_gauss.npy'
...
if __name__ == "__main__":
    ...
    for algorithm in algorithms:
        print("\n\n" + "="*80)
        print(f"RUNNING ALGORITHM: {algorithm.upper().replace('_', ' ')}")
        print("="*80)
        try:
            model, scaler, metrics = main(algorithm=algorithm)
            if metrics is not None:
                all_results[algorithm] = metrics
    ...
    print("="*80)
    print(f"{'Algorithm':<25} {'Accuracy':<12} {'Precision':<12} {'Recall':<12} {'F1-Score':<12}")
    print("-"*80)
    for algo, metrics in all_results.items():
        print(f"{algo.replace('_', ' ').title():<25} "
    ...
    if all_results:
        ...
        # Save as JSON
        json_path = os.path.join(results_dir, f'classification_results_{timestamp}.json')
        with open(json_path, 'w') as f:
            ...
            json.dump(convert_to_native_types(results_to_save), f, indent=2)
        print(f"\nResults saved to: {json_path}")
        
        # Save as CSV summary
        csv_path = os.path.join(results_dir, f'classification_results_{timestamp}.csv')
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Algorithm', 'Accuracy', 'Precision', 'Recall', 'F1-Score', 'N_Samples', 'N_R_Peaks', 'N_Predicted_R_Peaks'])
            for algo, metrics in all_results.items():
                writer.writerow([
                    algo,
