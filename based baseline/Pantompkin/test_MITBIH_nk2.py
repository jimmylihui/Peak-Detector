...
def neurokit2_detect(ecg: np.ndarray, fs: int = 360, method: str = 'pantompkins1985') -> np.ndarray:
    ...
    try:
        ...
        cleaned_ecg = nk.ecg_clean(ecg, sampling_rate=fs,method=method)

        # Detect R-peaks
        _, rpeaks = nk.ecg_peaks(cleaned_ecg, sampling_rate=fs, method=method)
...
def merge_close_peaks(peaks: np.ndarray, merge_distance: int) -> np.ndarray:
    """Merge nearby detections into a single median-centered peak."""
    ...
    for peak in peaks[1:]:
        if peak - clusters[-1][-1] <= merge_distance:
            clusters[-1].append(int(peak))
        else:
            clusters.append([int(peak)])

    merged = [int(np.median(cluster)) for cluster in clusters]
...
def neurokit2_detect_ensemble(ecg: np.ndarray, fs: int = 360) -> np.ndarray:
    ...
    # Merge nearby peaks produced by different detectors.
    return merge_close_peaks(np.array(sorted(all_peaks), dtype=int), merge_distance=int(0.1 * fs))
...
def calculate_hrv_from_peaks(peaks: np.ndarray, fs: int = 360, metric: str = 'sdnn') -> float:
    ...
    Args:
        peaks: Array of R-peak indices
        fs: Sampling frequency
        metric: HRV metric to calculate ('sdnn' or 'rmssd')
    
    Returns:
        HRV value in milliseconds, or np.nan if insufficient peaks
    """
    if peaks is None or len(peaks) < 2:
        return np.nan
...
def process_sample(args):
    ...
    if mode == 'ensemble':
        pred_peaks = neurokit2_detect_ensemble(preprocess_signal(sig), fs=fs)
    else:
        pred_peaks = neurokit2_detect(preprocess_signal(sig), fs=fs, method='pantompkins1985')
    inference_time = time.time() - inference_start

    # Convert to binary format using the actual sample length.
    binary_peaks = np.zeros(len(sig), dtype=np.float32)
    valid_peaks = pred_peaks[(pred_peaks >= 0) & (pred_peaks < len(sig))]
...
def evaluate_dataset(X_data, y_data, fs, tolerance_samples, detection_mode, n_cpus):
    """Evaluate a dataset split and return aggregate metrics and binary peaks."""
    args_list = [(i, X_data[i], y_data[i], fs, tolerance_samples, detection_mode) for i in range(X_data.shape[0])]

    # Process in parallel
    with Pool(n_cpus) as pool:
        results = list(tqdm(pool.imap(process_sample, args_list), total=len(args_list), desc='Evaluating test set'))
...
def main():
    ...
    assert X_test.shape[1] == y_test.shape[1], 'Signal and label lengths must match'
    window_length = X_test.shape[1]
    ...
    print(f'{"="*80}')
    result = evaluate_dataset(X_test, y_test, fs, tolerance_samples, detection_mode, n_cpus)

    print(f"Precision: {result['precision']:.4f}")
    print(f"Recall:    {result['recall']:.4f}")
    print(f"F1-Score:  {result['f1']:.4f}")
    print(f"Heart Rate MAE (BPM): {result['hr_mae']:.2f}" if not np.isnan(result['hr_mae']) else "Heart Rate MAE (BPM): NaN")
    print(f"Heart Rate MAPE (%): {result['hr_mape']:.2f}" if not np.isnan(result['hr_mape']) else "Heart Rate MAPE (%): NaN")
    print(f"HRV SDNN MAE (ms): {result['hrv_sdnn_mae']:.2f}" if not np.isnan(result['hrv_sdnn_mae']) else "HRV SDNN MAE (ms): NaN")
    print(f"HRV SDNN MAPE (%): {result['hrv_sdnn_mape']:.2f}" if not np.isnan(result['hrv_sdnn_mape']) else "HRV SDNN MAPE (%): NaN")
    print(f"HRV RMSSD MAE (ms): {result['hrv_rmssd_mae']:.2f}" if not np.isnan(result['hrv_rmssd_mae']) else "HRV RMSSD MAE (ms): NaN")
    print(f"HRV RMSSD MAPE (%): {result['hrv_rmssd_mape']:.2f}" if not np.isnan(result['hrv_rmssd_mape']) else "HRV RMSSD MAPE (%): NaN")
    print(f"\nTotal Inference Time: {result['total_inference_time']:.2f} seconds")
    print(f"Avg Inference Time per Sample: {result['avg_inference_time_per_sample']:.6f} seconds")
    print(f"Inference Throughput: {result['inference_throughput']:.2f} samples/sec")
    print(f"\nTotal on held-out test set: TP={result['tp']}, FP={result['fp']}, FN={result['fn']}")

    # Save predicted peaks as a dense binary array with the true window length.
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'MITBIH_predicted_peaks.npy')
    predicted_peaks_array = np.stack(result['binary_peaks']).astype(np.float32)
