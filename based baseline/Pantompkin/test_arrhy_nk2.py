...
def process_sample(args):
    i, sig, lbl, fs, tol, mode, window_length = args
    ...
    binary_peaks = np.zeros(window_length, dtype=np.int8)
    valid_peaks = pred_peaks[pred_peaks < window_length]  # Only keep peaks within range
...
def evaluate_fold(X_fold, y_fold, fs, tolerance_samples, detection_mode, n_cpus, window_length):
    """Evaluate a single fold and return metrics and binary peaks"""
    args_list = [(i, X_fold[i], y_fold[i], fs, tolerance_samples, detection_mode, window_length) for i in range(X_fold.shape[0])]
...
def main():
    ...
    x_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/X_train.npy'
    y_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/y_train.npy'
    x_test_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/X_test.npy'
    y_test_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_Arrhythmia/y_test.npy'
    ...
    fold_metrics = []
    all_predicted_peaks_binary = []  # Store binary predicted peaks for all folds
    for fold_idx in range(n_folds):
        ...
        # Evaluate fold
        fold_result = evaluate_fold(X_fold, y_fold, fs, tolerance_samples, detection_mode, n_cpus, window_length)
    ...
    # Save predicted peaks binary array
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'predicted_peaks.npy')
    predicted_peaks_array = np.array(all_predicted_peaks_binary, dtype=np.int8)
    np.save(output_file, predicted_peaks_array)
