#!/usr/bin/env python3
"""
Test ORIGINAL Pan-Tompkins R-peak detection on held-out test set.

Uses the Fabrizio1994/ECGClassification implementation via:
  /path/to/workspace/Pan-Tompkins-Plus-Plus/algos/pan_tompkins_original.py
"""
...
def pt_orig_detect(ecg: np.ndarray, fs: int = 360) -> np.ndarray:
    ecg = np.ascontiguousarray(ecg).astype(np.float64)
    detector = Pan_Tompkins_Original()
    try:
        peaks = detector.rpeak_detection(ecg, fs)
    except Exception:
...
def extract_gt_peaks_from_label(gt_label, fs=360):
...
def process_sample(args):
    ...
    pred_peaks = pt_orig_detect(sig, fs=fs)
...
def main():
    ...
    n_cpus = cpu_count()
    print(f"Using ORIGINAL Pan-Tompkins detector (Fabrizio1994)")
    ...
    print(f'\n{"="*80}')
    print('ORIGINAL PAN-TOMPKINS HELD-OUT TEST RESULTS')
    print(f'{"="*80}')
    ...
    hr_mape_val = float(np.mean(hr_mapes)) if hr_mapes else float('nan')
    sdnn_mae = float(np.mean(sdnn_errors)) if sdnn_errors else float('nan')
    sdnn_mape_val = float(np.mean(sdnn_mapes)) if sdnn_mapes else float('nan')
    rmssd_mae = float(np.mean(rmssd_errors)) if rmssd_errors else float('nan')
    rmssd_mape_val = float(np.mean(rmssd_mapes)) if rmssd_mapes else float('nan')
    ...
    print(f"Heart Rate MAPE (%): {fmt(hr_mape_val)}")
    print(f"HRV SDNN MAE (ms): {fmt(sdnn_mae)}")
    print(f"HRV SDNN MAPE (%): {fmt(sdnn_mape_val)}")
    print(f"HRV RMSSD MAE (ms): {fmt(rmssd_mae)}")
    print(f"HRV RMSSD MAPE (%): {fmt(rmssd_mape_val)}")
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'MITBIH_predicted_peaks_pt_orig.npy')
