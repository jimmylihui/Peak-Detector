...
def neurokit2_detect_ensemble(ecg: np.ndarray, fs: int = 360) -> np.ndarray:
    ...
    while i < len(all_peaks):
        cluster = [all_peaks[i]]
        j = i + 1
        while j < len(all_peaks) and all_peaks[j] - all_peaks[i] <= tolerance:
            cluster.append(all_peaks[j])
            j += 1
        
        # Use median of cluster if multiple detectors agree
        if len(cluster) >= 2:
            clustered_peaks.append(int(np.median(cluster)))
        
        i = j
    
    return np.array(clustered_peaks, dtype=int)
