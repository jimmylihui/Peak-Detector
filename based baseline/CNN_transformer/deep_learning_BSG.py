...
class FRNetTrainer:
    ...
    def load_dataset(self):
        ...
        X_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BSG/BCG_train.npy')
        from sklearn.model_selection import train_test_split
        
        y_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BSG/Corrected_peaks_train_gaussian.npy')
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100, batch_size=32, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 'best_frnet_bsg_model.pt')
                patience = 0
            else:
                patience += 1
                if patience >= 15:
        ...
        self.model.load_state_dict(torch.load('best_frnet_bsg_model.pt'))
        return history
...
def main():
    ...
    history = trainer.train(X_train, y_train, X_val, y_val, 
                           epochs=1000, batch_size=32, lr=0.001)
    
    trainer.plot_history(history)
    
    print("\nTraining Complete!")
    print(f"Best model saved to: best_frnet_bsg_model.pt")
    return history
