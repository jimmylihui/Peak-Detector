...
def calculate_metrics(pred_peaks, gt_peaks, tolerance=0.03, fs=125):
    """
...
class FRNetTrainer:
    ...
    def load_dataset(self):
        ...
        X_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/PPG/processed/X_train.npy')
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100, batch_size=32, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 'best_frnet_BIDMC_model.pt')
                patience = 0
            else:
                patience += 1
                if patience >= 15:
        ...
        self.model.load_state_dict(torch.load('best_frnet_BIDMC_model.pt'))
        return history
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'FRNet_BIDMC_predicted_peaks.npy')
...
def main_cross_validation():
    ...
    print("FR-Net ECG R-peak Detection - Cross-Validation Pipeline (BIDMC)")
    print("="*70)
