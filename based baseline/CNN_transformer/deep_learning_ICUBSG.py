...
class FRNetTrainer:
    ...
    def load_dataset(self):
        ...
        X_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/X_train.npy')
        from sklearn.model_selection import train_test_split
        
        y_train=np.load('/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/y_train.npy')
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=100, batch_size=32, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 'best_frnet_icubs_model.pt')
                patience = 0
            else:
                patience += 1
                if patience >= 15:
        ...
        self.model.load_state_dict(torch.load('best_frnet_icubs_model.pt'))
        return history
    ...
    def cross_validate(self, epochs=5, batch_size=32, learning_rate=0.001):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'FRNet_ICUBSG_predicted_peaks.npy')
...
def main():
    ...
    print("\nTraining Complete!")
    print(f"Best model saved to: best_frnet_icubs_model.pt")
    return history
...
def main_cross_validation():
    ...
    print("FR-Net ECG R-peak Detection - Cross-Validation Pipeline (ICUBSG)")
    print("="*70)
