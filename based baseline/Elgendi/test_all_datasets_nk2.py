#!/usr/bin/env python3
"""
NeuroKit2 Elgendi benchmark on all datasets — for comparison with standalone Elgendi.

Uses NeuroKit2's built-in Elgendi implementation:
  - ECG: nk.ecg_clean + nk.ecg_peaks with method='elgendi2010'
  - PPG/BCG: nk.ppg_clean + nk.ppg_peaks with method='elgendi'

Same tolerance, datasets, and metrics as test_all_datasets.py (standalone).
"""
...
ABSOLUTE_TOL_SEC = 0.05
RELATIVE_TOL_FRAC = 0.05
RELATIVE_TOL_MIN = 5


# ---- Detector ----

def detect_nk2_elgendi(sig_data, fs, modality):
    """NeuroKit2 Elgendi detector — ECG or PPG pipeline depending on modality."""
    sig_data = np.ascontiguousarray(sig_data).astype(np.float64)
    try:
        if modality == 'ecg':
            cleaned = nk.ecg_clean(sig_data, sampling_rate=fs, method='elgendi2010')
            _, info = nk.ecg_peaks(cleaned, sampling_rate=fs, method='elgendi2010')
            peaks = info.get('ECG_R_Peaks', [])
        else:
            cleaned = nk.ppg_clean(sig_data, sampling_rate=fs)
...
def _refractory_filter(peaks, fs):
    """Remove peaks closer than 200 ms."""
    refractory = 0.200 * fs
    corrected = []
    skip_next = False
...
def _worker(args):
    sig_data, lbl, fs, abs_tol, modality = args
    gt = extract_gt_peaks(lbl, fs)

    t0 = time.time()
    pred = detect_nk2_elgendi(sig_data, fs, modality)
    inf_time = time.time() - t0

    valid = pred[(pred >= 0) & (pred < len(sig_data))]
    binary = np.zeros(len(sig_data), dtype=np.float32)
    if len(valid) > 0:
        binary[valid] = 1.0
...
def run_dataset(ds, n_cpus):
    ...
    args = [(X[i], Y[i], fs, abs_tol, modality) for i in range(X.shape[0])]
...
def main():
    ...
    for ds in DATASETS:
        print(f"\n  Dataset: {ds['name']} (fs={ds['fs']}, modality={ds['modality']})")
        out = run_dataset(ds, n_cpus)
        if out is None:
            continue
        agg, binaries = out
        ...
        print(f"      Precision={a['precision']:.4f} Recall={a['recall']:.4f} F1={a['f1']:.4f}")
    ...
    # Save NK2 JSON
    json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results_nk2.json')
    with open(json_path, 'w') as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nJSON results saved to: {json_path}")

    # Merge all three: Pan-Tompkins, standalone Elgendi, NK2 Elgendi
    write_combined_result_md(all_results)


def write_combined_result_md(nk2_results):
    """Merge all results into result.md."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    pt_json = os.path.join(base_dir, '..', 'Pantompkin', 'results.json')
    pt_results = {}
    if os.path.exists(pt_json):
        with open(pt_json) as f:
            pt_results = json.load(f)

    standalone_json = os.path.join(base_dir, 'results.json')
    standalone_results = {}
    if os.path.exists(standalone_json):
        with open(standalone_json) as f:
            standalone_results = json.load(f)

    md_path = os.path.join(base_dir, '..', 'result.md')
    lines = []
    ...
    algos = []
    if pt_results:
        algos.append(('pt_orig', 'Pan-Tompkins', pt_results))
    if standalone_results:
        algos.append(('elgendi', 'Elgendi (Standalone)', standalone_results))
    algos.append(('elgendi_nk2', 'Elgendi (NeuroKit2)', nk2_results))

    for det_key, det_label, results in algos:
        ...
        for ds in DATASETS:
            ...
            tput = f"{1.0/avg_t:.0f}/s" if avg_t > 0 else "N/A"
            lines.append(
    ...
    with open(md_path, 'w') as f:
        f.write(md_content)
    print(f"\nCombined results written to: {md_path}")
