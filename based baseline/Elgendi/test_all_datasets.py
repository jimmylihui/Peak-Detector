#!/usr/bin/env python3
"""
Elgendi (Two Moving Average) benchmark on all datasets.

Uses the standalone implementation from py-ecg-detectors:
...
def detect_elgendi(ecg, fs):
    """Elgendi two-moving-average detector from py-ecg-detectors."""
...
def _worker(args):
    sig_data, lbl, fs, abs_tol = args
    gt = extract_gt_peaks(lbl, fs)

    t0 = time.time()
    pred = detect_elgendi(sig_data, fs)
...
def run_dataset(ds, n_cpus):
    ...
    args = [(X[i], Y[i], fs, abs_tol) for i in range(X.shape[0])]
...
def main():
    ...
    print(f"\n{'#'*80}")
    print(f"  Detector: {det_label} (Two Moving Average, py-ecg-detectors)")
    print(f"{'#'*80}")
    ...
    # Save raw JSON for Elgendi
    json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results.json')
    with open(json_path, 'w') as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nJSON results saved to: {json_path}")

    # Now merge with Pan-Tompkins results and write combined result.md
    write_combined_result_md(all_results)


def write_combined_result_md(elgendi_results):
    """Merge Elgendi results with existing Pan-Tompkins results into result.md."""
    pt_json = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           '..', 'Pantompkin', 'results.json')
    pt_results = {}
    ...
    algos.append(('elgendi', 'Elgendi', elgendi_results))
