...
def main():
    ...
    x_test_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BSG/19.npy'
    # Set y_test_path to a valid file path if labels are available; leave undefined or comment out for inference-only
    ...
    y_test = None
    has_labels = False
    if 'y_test_path' in locals() and isinstance(locals().get('y_test_path'), str) and os.path.exists(locals().get('y_test_path')):
        y_test = np.load(locals().get('y_test_path'))
        has_labels = True
    if has_labels:
    ...
    for i in tqdm(range(X_test.shape[0]), desc='Testing with Choi'):
        ...
        # Only compute metrics if labels are available and there are predicted peaks
        if has_labels and len(pred_peaks) > 0:
            # Detection metrics
    ...
    # Aggregate metrics (only when labels exist)
    if has_labels:
        ...
        print('\nNo labels provided. Skipping metric calculations. Saving predictions only.')

    # Save predicted peaks as binary array with shape (n, 1000)
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'BSG_predicted_peaks.npy')
