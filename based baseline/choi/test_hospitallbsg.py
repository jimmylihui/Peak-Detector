...
def calculate_detection_metrics(pred_peaks: np.ndarray, gt_peaks: np.ndarray, tolerance_percent: float = 0.05) -> dict:
    # Calculate dynamic tolerance based on median RR interval
    if len(gt_peaks) > 1:
        gt_rr_intervals = np.diff(gt_peaks)
        median_rr = np.median(gt_rr_intervals)
        tolerance_samples = max(int(tolerance_percent * median_rr), 5)
    else:
...
def process_sample(args):
    ...
    if len(pred_peaks) > 0:
        m = calculate_detection_metrics(pred_peaks, gt_peaks, tol_percent)
...
def evaluate_fold(X_fold, y_fold, fs, tolerance_percent, n_cpus):
    """Evaluate a single fold and return metrics and binary peaks"""
    args_list = [(i, X_fold[i], y_fold[i], fs, tolerance_percent) for i in range(X_fold.shape[0])]
...
def main():
    ...
    x_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/X_train.npy'
    y_train_path = '/path/to/workspace/project-BCG-LLM/combined_data/combined_splitted_data/BCG_hospital/y_train.npy'
    ...
    for fold_idx in range(n_folds):
        ...
        fold_result = evaluate_fold(X_fold, y_fold, fs, tolerance_percent, n_cpus)
